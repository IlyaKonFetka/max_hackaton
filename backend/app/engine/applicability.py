...
`{field, eq|ne|in|nin|gt|gte|lt|lte}`. Любой предикат читается вслух — это сознательное ограничение.

Кроме ответа «применимо / нет» движок объясняет, ПОЧЕМУ пункт не применим:
собирает листья, которые не выполнились, и переводит их в человеческие фразы
через подписи полей профиля. Это то, чего нет ни у одного аналога.
"""
